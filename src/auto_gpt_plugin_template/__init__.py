...
## One Shot Python Plugin Example

I am using a tool called polywrap that allows you to define a schema in GraphQL and will generate the types, interfaces, enums, etc in the target language (Ex: typescript, python, rust, etc).

Polywrap maps types from GraphQL schema to Python in following way.

`type Module` is an abstract class that defines the interface of the plugin
Other types are translated to`TypedDict` in python
Enum are translated to enum in both languages
Codegen for python uses typehint to give typesafe code in python.

Example:
- GraphQL schema:
```graphql
type Module {
    # sampleMethod expects string as an argument and suffixes ` from sampleMethod` and return as output
    sampleMethod(data: String!): String!
}
```
...
class Module(Generic[TConfig], PluginModule[TConfig]):
    ...
    async def sample_method(
        ...
        pass
```
Note: The above code is generated by the polywrap and WrapGPT doesn't need to write it since it will be available as wrap module in the current directory after running `polywrap codegen` command and can be imported in the target language.

here's the actual implementation of the method
```
class SampleModule(Module):
    def sample_method(self, args: ArgsSampleMethod, client: object, env: None):
        return f"{args['data']} from sampleMethod"
```

> Note casing in GraphQL and invocations will always be camelCase and in python implementation, it will be snake_case

If you understand the structure then reply with 'Understood! Please give me the GraphQL schema of the program you want us to implement:'

## Actual plugin module

Here's the GraphQL schema of the Logging plugin I want in the python langauge
...
```
If you understand the schema of the logger plugin reply with `I understood!`
